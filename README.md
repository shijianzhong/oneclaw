...
打开 **设置 → 外观显示 → Live2D 桌面宠物** 开关，即时显示或隐藏。Live2D 窗口与主聊天窗口相互独立，可同时显示。
...
**Q: Live2D 和主聊天窗口可以同时显示吗？**
A: 可以。两个窗口相互独立，通过 **设置 → 外观显示 → Live2D 桌面宠物** 开关控制，开关状态与主窗口是否打开无关。
...
Toggle **Settings → Appearance → Live2D Desktop Pet** to show or hide instantly. The Live2D window is independent of the main chat window — both can be visible at the same time.
...
> 💡 Voice features require speech model files (~200MB). See [Speech Model Setup](#-speech-model-setup-1) below.
...
OneClaw (Electron)
  ...
  └── 🎭 Live2D window         (transparent overlay, independent of chat window)
```
...
**Q: Can Live2D and the main chat window be shown at the same time?**
A: Yes. The two windows are independent. Toggle Live2D via **Settings → Appearance → Live2D Desktop Pet** — it stays visible regardless of whether the chat window is open.
